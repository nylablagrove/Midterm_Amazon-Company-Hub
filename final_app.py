...
# read the required sales file
# if it is missing we show a clear error and stop the app so the user can fix it
sales = safe_read_csv("amazon_sales_with_two_months.csv")
if sales is None:
    st.error("Missing **amazon_sales_with_two_months.csv**. Place it next to app.py and reload.")
    st.stop()

# read optional reviews file if available
reviews = safe_read_csv("amazon_reviews.csv")

# read optional product metadata if available
catalog = safe_read_csv("amazon_products.csv")
